...
Y_trn = torch.from_numpy(Y_trn.astype(float)).type(torch.FloatTensor)
Y_tst = torch.from_numpy(Y_tst.astype(float)).type(torch.FloatTensor)
Y_val = torch.from_numpy(Y_val.astype(float)).type(torch.FloatTensor)
...
if __name__ == "__main__":
    ...
    load_model()
    Y_tst_pred = HSI.model(X_tst)
    HSI_tst_loss = HSI.criterion(Y_tst_pred, Y_tst)
    HSI_tst_loss = HSI_tst_loss.detach().numpy()
    
    print("The MSE Loss for test set prediction by ANN model is", HSI_tst_loss)
    
    Y_mean_abs = np.mean(list(map(abs, HSI_Y.tolist())))
    print(HSI_tst_loss/Y_mean_abs^2)
