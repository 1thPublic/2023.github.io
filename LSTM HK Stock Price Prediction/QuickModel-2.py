...
class RNN(nn.Module):
    def init_model(self, input_size, hidden_size, num_layers, num_classes, learn_rate):
        super().__init__()
    ...
    def train(self, x_trn, y_trn, x_val, y_val, epoch):
        ...
        for j in range(epoch):
            ...
            y_val_out = self.fc(y_val_out[:, -1, :])
            val_loss = self.criterion(y_val_out, y_val[i])
            RNN_iter_loss.append(val_loss.item())
...
if __name__ == "__main__":
    ...
    n_epoch = [1000, 2000, 3000, 4000, 5000]
    hidden_layer = 2
    RNN_all_loss = []
    ...
    for i in n_hidden:
        ...
        RNN.train(x_trn = X_trn, y_trn = Y_trn, 
                  ...
                  epoch = 10)
        RNN_all_loss.extend(RNN_iter_loss)
        
    best_hl = n_hidden[np.argmin(RNN_all_loss) // 10]
    best_ep = n_epoch[np.argmin(RNN_all_loss) % 10]
    ...
    best_lr = learn_rate[np.argmin(RNN_iter_loss)]
